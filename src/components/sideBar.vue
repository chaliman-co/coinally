<template>
    <div v-if="global.user" class="dashboard__sidebar">
        <span class="hamburger">
            <img src="~img/close.svg" alt="Close">
        </span>
        <img :src="`${global.apiRootUrl}${global.user.imagePath}`" alt="Avatar" class="avatar">
        <div class="title">
        ...
        <div class="links">
            <router-link v-for="(link, index) in displayLinks" :key="index" :to="link.path" :class="$route.path == link.path? 'link active' : 'link'">
                ...
                <a href="" @click.prevent="logOut" class="link logout">
                    Logout
                </a>
        </div>
    </div>
