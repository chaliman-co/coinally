...
const
    ...
    getLive = function getLive(cb) {
        assetModel.find({}).then((assets) => {
            assets.forEach(asset => liveAssets[asset.code] = asset);
        ...
        async.parallel(
            assets.map(asset => function(next) {
                ...
                for (const otherAssetCode in liveAssets) {
                    if (otherAssetCode == asset.code) continue;
...
function calculateRate(asset, otherAsset) {
    if (asset.type == 'digital' && otherAsset.type == 'fiat')
        ...
        return handleDigitalToDigital(asset, otherAsset);
    else if (asset.type === 'card' && otherAsset.type === 'digital')
        return handleCardToDigital(asset, otherAsset);
...
function handleDigitalToDigital(asset, otherAsset) {
    return getRateOnline(asset.code, otherAsset.code);
}
function handleDigitalToFiat(digitalAsset, fiatAsset, cb) {//console.log(util.inspect({asset: digitalAsset._doc, otherAset: fiatAsset._doc}, {depth: null}))
    return new Promise((resolve, reject) => {
        if (isNaira(fiatAsset)) {
            getDollarRate(digitalAsset)
                .then(rate => {
                    let nairaRate = rate / fiatAsset.price_usd;
                    let paddedRate = (1 - 0.01 * digitalAsset.buyingProfit) * nairaRate; //console.log({paddedRate})
                    resolve(paddedRate);
                })
...
const queryableRates = ["ethbtc", "btcusd", "ethusd"]
function getRateOnline(fromCode, toCode) {
    let [query, reversed] = [`${fromCode.toLowerCase()}${toCode.toLowerCase()}`, false];
    if (! ~queryableRates.indexOf(query)) [query, reversed] = [`${toCode.toLowerCase()}${fromCode.toLowerCase()}`, true];
    const url = `https://www.bitstamp.net/api/v2/ticker/${query}`;
    return new Promise((resolve, reject) => {
        axios.get(url)
            .then(response => resolve(reversed? 1/response.data.last : response.data.last))
            .catch(err => reject(err));
    })
}
