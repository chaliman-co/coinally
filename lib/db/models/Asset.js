...
function calculateRate(asset, otherAsset, cb) {
    const conversionFunc = new Function(otherAsset.code, 'convertToDollars', 'getOnlineRate', 'cb', asset.conversionTable[otherAsset.code]).bind(asset);
    return conversionFunc(otherAsset, convertToDollars, getOnlineRate, (rate) => {
        ...
        if (asset.type == 'fiat' || otherAsset.type == 'fiat') {
            const reverseConversionFunc = new Function(asset.code, 'convertToDollars', 'getOnlineRate', 'cb', otherAsset.conversionTable[asset.code]).bind(otherAsset);
