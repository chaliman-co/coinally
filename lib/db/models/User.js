...
userSchema.pre('validate', function (next) {
  for (const vDetail of this.verificationDetails) {
    const index = this.verificationDetails.indexOf(vDetail); console.log('detail: ', index, vDetail);
    if (!vDetail) continue;
    if (vDetail.type == 'bvn') {
      // console.log(vDetail, !vDetail.imagePath && /^\d{11}$/.test(vDetail.number))
      if (vDetail.imagePath || !/^\d{11}$/.test(vDetail.number)) this.invalidate(`verificationDetails[${index}]`, 'verification detail invalid');
    }
    else { // console.log(vDetail, !vDetail.number && !!vDetail.imagePath);
      if (!vDetail.imagePath) this.invalidate(`verificationDetails[${index}]`, 'verification detail invalid');
 }
  }
  return next();
});
