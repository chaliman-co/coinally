...
const transactionSchema = mongoose.Schema({
    ...
    createdAt: {
        ...
        validate: {
            ...
            message: 'creation time not valid',
        },
    },
    depositAssetCode: {
        type: String,
    ...
    status: {
        type: String,
        enum: ['failed', 'awaiting payment', 'payment received', 'pending', 'completed'],
    ...
    receiptAddress: {
        ...
        required: [true, 'receipt address not provided'],
    },
    refundAddress: {
        type: String,
        validate: {
            validator: function validateAddress(address, cb) { // address is either an array index indicating which of the user's asset account to send money to or an address url
                try {
                    if (this.depositAsset.type == 'fiat') return this.user.assetAccounts[address] && typeof(this.user.assetAccounts[address]).address === 'object'; // check if the index exists and is a bank account
                    if (this.depositAsset.type == 'digital') return true;
                } catch (err) { // Either this.depositAsset or this.user is undefined
                    return false;
                }
            },
            message: 'refund address invalid',
        },
        required: [true, 'refund address not provided'],
    },
    updatedAt: {
        type: Date,
...
transactionSchema.pre('validate', function applyVerificationRule(next) {
    ...
    this.rate = this.depositAsset && this.receiptAddress ? Asset.convert(this.depositAsset.code, this.receiptAsset.code) : NaN;
    this.receiptAmount = this.depositAmount * this.rate; // console.log('rate: ', this.rate, '\nreceipt amount: ', this.receiptAmount)
    User.findOne({
        _id: this.user,
    }).then((user) => { this.user = user, next(); }, (err) => { console.log(err), this.user = null, next(); });
});
