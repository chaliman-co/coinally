...
function extend(server) {
    server.once('request', (req, res) => {
        const resProto = res.constructor.prototype;
...
function getAuthToken(user) {
    ...
    const options = {
        expiresIn: serverConfig.TOKEN_VALIDITY,
    };

    const authToken = jwt.sign(payload, serverConfig.TOKEN_SECRET, options);
...
function sendAuthCode(user, cb) {
    ...
    const code = user.verificationCode.value;

    emailer.sendAuthCode(user, code, cb);
}
